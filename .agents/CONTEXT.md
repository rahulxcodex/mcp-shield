...
- **`mcp-shield`** (Public GitHub `rahulxcodex/mcp-shield`, npm package `mcpshld` v1.0.17):
  ...
  - Open Security Benchmark Suite: 42 suites / 693 tests covering AST evasion, DLP, memory bounds, and protocol invariants.
...
- **`mcp-shield-enterprise-intel`** (Private GitHub `rahulxcodex/mcp-shield-enterprise-intel`, Render Web Service):
  ...
  - Endpoint: `https://mcp-shield-enterprise-intel.onrender.com/api/v1/intel/scoring`.
...
- **`mcp-shield-licensing`** (Private GitHub `rahulxcodex/mcp-shield-licensing`, Vercel App):
  ...
  - Endpoint: `https://mcp-shield-licensing.vercel.app`.

## 2. Trade Secret Invariants & Security Boundaries
- **No Private Keys in Open Source**: Ed25519 signing private keys only exist in Vercel KMS/env (`LICENSE_PRIVATE_KEY`).
- **No Plaintext Master Keys in Code**: Master license keys verified strictly via one-way SHA-256 cryptographic hashes.
- **Strict Separation of Scoring Weights**: Proprietary non-linear scoring stays isolated in `mcp-shield-enterprise-intel`.
- **Environment Isolation**: All `.env*` files strictly gitignored and excluded from npm packages.

## 3. Deployment & Release Matrix
- **npm**: `npm publish` pushes `mcpshld` package to npm registry.
- **GitHub**: All 3 repos push to `origin main`.
- **Vercel**: Auto-deploys `mcp-shield-dashboard` and `mcp-shield-licensing`.
- **Render**: Auto-deploys `mcp-shield-enterprise-intel`.
