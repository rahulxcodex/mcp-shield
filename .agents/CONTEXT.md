...
- **Enterprise Intel Hardening (`mcp-shield-enterprise-intel`)**: Opaque `POST /api/v1/decision` contract; trade-secret protection (403 Forbidden for standard clients on raw corpus/chains); scoped auth (`intel:decide`, `intel:corpus:read`, `intel:admin`); sliding-window rate limiting; Zod schema validation; mathematical score saturation [0, 100] and NaN neutralization; structured JSON audit logging; emergency circuit-breaker (`POST /api/v1/admin/service-disable`) and threat corpus rollback (`POST /api/v1/admin/corpus-rollback`); 15/15 tests passing.
- **Licensing Control Plane Hardening (`mcp-shield-licensing`)**: Replaced plaintext `key_hash` with SHA-256 verifiers; per-project HMAC telemetry auth with 5-minute clock-skew check and nonce replay defense; DLP privacy redactions on preview telemetry; server-authoritative Stripe billing lifecycle with fail-closed pricing (unknown prices strictly resolve to free tier); daily automated Stripe subscription reconciliation job (`scripts/reconcile-stripe-subscriptions.ts`); dedicated owner-transfer endpoint (`POST /api/v1/organizations/[id]/transfer-owner`); administrative break-glass APIs (`POST /api/v1/admin/break-glass`); signed Ed25519 policy manifests; 17/17 tests passing (6 production gate tests + 11 multi-tenant IDOR/RBAC tests).
- **MCP Shield Core Hardening (`mcp-shield`)**: Remote intel CircuitBreaker (`src/security/circuit-breaker.ts`) with high-risk fail-closed semantics; signed policy bundle sync; 7-component property-based fuzz suite (`scripts/fuzz.ts` covering JSON-RPC parser, MCP lifecycle, AST analyzer, policy matcher, DLP streaming, telemetry privacy, and intel validation); full 12-step customer journey E2E test (`tests/integration/full-e2e-journey.test.ts` - 12/12 passing); concurrency & race condition defense suite (`tests/security/race-concurrency.test.ts` - 8/8 passing); automated disaster recovery restoration drill (`scripts/disaster-recovery-drill.ts` - RPO 12m, RTO 0.006s); machine-readable certification report `reports/production-certification.json` certified.
