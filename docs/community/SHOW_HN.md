...
2. **Reversible (Bijective) Secret Sanitizer (DLP)**: Single-pass tokenization of high-entropy credentials (AWS keys, OpenAI/Anthropic keys, GitHub PATs, SSH keys) replaced with session tokens (`[[SHIELD_SECRET_...]]`) on the wire and losslessly restored on responses. Tested across 1,780+ lines of real code and logs with 100% Precision and 100% Recall.
